					...
					mem_addr_t status_addr)
{
	...
	memcpy(&eth_type,
		&header_buf[offsetof(struct net_eth_hdr, type)],
		sizeof(eth_type));
	...
	case NET_ETH_PTYPE_IP:
		memcpy(&len,
			&header_buf[sizeof(struct net_eth_hdr) +
			offsetof(struct net_ipv4_hdr, len)],
			sizeof(len));

		packet_size = ntohs(len) + sizeof(struct net_eth_hdr);
		if (packet_size < sizeof(struct net_eth_hdr) + sizeof(struct net_ipv4_hdr) ||
			packet_size > NET_ETH_MAX_FRAME_SIZE) {
			LOG_WRN("Invalid IPv4 packet size %zu", packet_size);
			goto out;
		}
	...
	case NET_ETH_PTYPE_IPV6:
		memcpy(&len,
			&header_buf[sizeof(struct net_eth_hdr) +
			offsetof(struct net_ipv6_hdr, len)],
			sizeof(len));
	...
	default:
		LOG_DBG("Unsupported ethertype 0x%04x", ntohs(eth_type));
		goto out;
	}

	const size_t header_len = MIN(sizeof(header_buf), packet_size);
	
	pkt = net_pkt_rx_alloc_with_buffer(data->iface, packet_size,
	...
	LOG_DBG("pkt=%p packet_size=%zu header_len=%zu first_frag=%p",
		pkt, packet_size, header_len, pkt->buffer);
	
	if (net_pkt_write(pkt, header_buf, header_len) < 0) {
		LOG_ERR("Could not write header to packet!");
		net_pkt_unref(pkt);
		goto out;
	}
